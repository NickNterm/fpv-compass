...
export default function IdeasBoard({ initialIdeas }: IdeasBoardProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    // Skip the first run: initial ideas are already seeded from the server.
    // Only refetch when the user changes sort/filter.
    if (!didMount.current) {
